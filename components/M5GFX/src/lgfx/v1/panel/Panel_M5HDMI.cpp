...
namespace lgfx
{
    inline namespace v1
    {
        ...
        Panel_M5HDMI::LOAD_FPGA::LOAD_FPGA(uint_fast8_t _TCK_PIN,
                                           uint_fast8_t _TDI_PIN,
                                           uint_fast8_t _TDO_PIN,
                                           uint_fast8_t _TMS_PIN)
        {
        ...
        {
            uint8_t buffer;
            lgfx::i2c::transactionWriteRead(this->HDMI_Trans_config.i2c_port,
                                            this->HDMI_Trans_config.i2c_addr,
                                            &register_address,
                                            1,
                                            &buffer,
                                            1,
                                            this->HDMI_Trans_config.freq_read);
            return buffer;
        }
        ...
        bool Panel_M5HDMI::HDMI_Trans::writeRegister(uint8_t register_address, uint8_t value)
        {
            ...
            int retry = 4;
            while (lgfx::i2c::transactionWrite(this->HDMI_Trans_config.i2c_port,
                                               this->HDMI_Trans_config.i2c_addr,
                                               buffer,
                                               2,
                                               this->HDMI_Trans_config.freq_write)
                       .has_error() &&
                   --retry)
            {
                lgfx::delay(1);
        ...
        Panel_M5HDMI::HDMI_Trans::ChipID Panel_M5HDMI::HDMI_Trans::readChipID(void)
        {
            ...
            lgfx::i2c::i2c_temporary_switcher_t i2c_switch{HDMI_Trans_config.i2c_port,
                                                           HDMI_Trans_config.pin_sda,
                                                           HDMI_Trans_config.pin_scl};

            if (this->writeRegister(0xff, 0x80) && this->writeRegister(0xee, 0x01))
            {
        ...
        void Panel_M5HDMI::HDMI_Trans::reset(void)
        {
            lgfx::i2c::i2c_temporary_switcher_t i2c_switch{HDMI_Trans_config.i2c_port,
                                                           HDMI_Trans_config.pin_sda,
                                                           HDMI_Trans_config.pin_scl};
            static constexpr const uint8_t data[] = {0xff, 0x81, 0x30, 0x00, 0x02, 0x66, 0x0a, 0x06, 0x15, 0x06,
                                                     0x4e, 0xa8, 0xff, 0x80, 0xee, 0x01, 0x11, 0x00, 0x13, 0xf1,
                                                     0x13, 0xf9, 0x0a, 0x80, 0xff, 0x82, 0x1b, 0x77, 0x1c, 0xec,
                                                     0x45, 0x00, 0x4f, 0x40, 0x50, 0x00, 0x47, 0x07};
            this->writeRegisterSet(data, sizeof(data));
            i2c_switch.restore();
        }

        Panel_M5HDMI::HDMI_Trans::HDMI_Trans(const lgfx::Bus_I2C::config_t& i2c_config) { HDMI_Trans_config = i2c_config; }
        ...
        bool Panel_M5HDMI::HDMI_Trans::init(void)
        {
            ...
            {
                // 96kHz audio setting.
                //    static constexpr const uint8_t data_1[] = { 0xff, 0x82, 0xD6, 0x8E, 0xD7, 0x04, 0xff, 0x84, 0x06, 0x08,
                //    0x07, 0x10, 0x09, 0x00, 0x0F, 0xAB, 0x34, 0xD5, 0x35, 0x00, 0x36, 0x30, 0x37, 0x00, 0x3C, 0x21,
                //                                                0xff, 0x82, 0xde, 0x00, 0xde, 0xc0, 0xff, 0x81, 0x23, 0x40,
                //                                                0x24, 0x64, 0x26, 0x55, 0x29, 0x04, 0x4d, 0x00, 0x27, 0x60,
                //                                                0x28, 0x00, 0x25, 0x01, 0x2c, 0x94, 0x2d, 0x99 };

                // 48kHz audio setting.
                static constexpr const uint8_t data_1[] = {
                    0xff, 0x82, 0xD6, 0x8E, 0xD7, 0x04, 0xff, 0x84, 0x06, 0x08, 0x07, 0x10, 0x09, 0x00, 0x0F, 0x2B, 0x34, 0xD5,
                    0x35, 0x00, 0x36, 0x18, 0x37, 0x00, 0x3C, 0x21, 0xff, 0x82, 0xde, 0x00, 0xde, 0xc0, 0xff, 0x81, 0x23, 0x40,
                    0x24, 0x64, 0x26, 0x55, 0x29, 0x04, 0x4d, 0x00, 0x27, 0x60, 0x28, 0x00, 0x25, 0x01, 0x2c, 0x94, 0x2d, 0x99};

                // disable audio setting.
                //    static constexpr const uint8_t data_1[] = { 0xff, 0x82, 0xde, 0x00, 0xde, 0xc0, 0xff, 0x81, 0x23, 0x40,
                //    0x24, 0x64, 0x26, 0x55, 0x29, 0x04, 0x4d, 0x00, 0x27, 0x60, 0x28, 0x00, 0x25, 0x01, 0x2c, 0x94, 0x2d, 0x99
                //    };
            ...
            if (id.id[2] == 0xE2)
            {
                static constexpr const uint8_t data_u3[] =
                    {0x4d, 0x09, 0x27, 0x66, 0x28, 0x88, 0x2a, 0x00, 0x2a, 0x20, 0x25, 0x00, 0x2c, 0x9e, 0x2d, 0x99};
            ...
            for (int i = 0; i < 8; ++i)
            {
                static constexpr const uint8_t data_pll[] =
                    {0xff, 0x80, 0x16, 0xf1, 0x18, 0xdc, 0x18, 0xfc, 0x16, 0xf3, 0x16, 0xe3, 0x16, 0xf3, 0xff, 0x82};
                ...
                if (locked && done && value != 0xff)
                {
                    static constexpr const uint8_t data[] = {
                        0xb9, 0x00, 0xff, 0x84, 0x43, 0x31, 0x44, 0x10, 0x45, 0x2a, 0x47, 0x04, 0x10, 0x2c,
                        0x12, 0x64, 0x3d, 0x0a, 0xff, 0x80, 0x11, 0x00, 0x13, 0xf1, 0x13, 0xf9, 0xff, 0x81,
                        0x31, 0x44, 0x32, 0x4a, 0x33, 0x0b, 0x34, 0x00, 0x35, 0x00, 0x36, 0x00, 0x37, 0x44,
                        0x3f, 0x0f, 0x40, 0xa0, 0x41, 0xa0, 0x42, 0xa0, 0x43, 0xa0, 0x44, 0xa0, 0x30, 0xea};
                    this->writeRegisterSet(data, sizeof(data));
                    result = true;
                    break;
                }
        ...
        size_t Panel_M5HDMI::HDMI_Trans::readEDID(uint8_t* EDID, size_t len)
        {
            ...
            {
                static constexpr const uint8_t data2[2][6] = {{0x07, 0x36, 0x07, 0x34, 0x07, 0x37},
                                                              {0x07, 0x76, 0x07, 0x74, 0x07, 0x77}};
        ...
        {
            ESP_LOGI(TAG,
                     "i2c port:%d sda:%d scl:%d",
                     _HDMI_Trans_config.i2c_port,
                     _HDMI_Trans_config.pin_sda,
                     _HDMI_Trans_config.pin_scl);
            ...
            do
            {
                // ESP_LOGI(TAG, "FREQ:%lu , %lu  DIV_W:%lu , %lu", _bus->getClock(), _bus->getReadClock(), div_write,
                // div_read);
        ...
        bool Panel_M5HDMI::_init_resolution(void)
        {
            ...
            {
                ESP_LOGD(TAG,
                         "PLL feedback_div:%d  input_div:%d  output_div:%d  OUTPUT_CLOCK:%d",
                         vc.feedback_divider,
                         vc.input_divider,
                         vc.output_divider,
                         (int)OUTPUT_CLOCK);
                ...
                ESP_LOGD(TAG,
                         "video timing(Hori) total:%d active:%d frontporch:%d sync:%d backporch:%d",
                         vt.h.active + vt.h.front_porch + vt.h.sync + vt.h.back_porch,
                         vt.h.active,
                         vt.h.front_porch,
                         vt.h.sync,
                         vt.h.back_porch);
                ESP_LOGD(TAG,
                         "video timing(Vert) total:%d active:%d frontporch:%d sync:%d backporch:%d",
                         vt.v.active + vt.v.front_porch + vt.v.sync + vt.v.back_porch,
                         vt.v.active,
                         vt.v.front_porch,
                         vt.v.sync,
                         vt.v.back_porch);
        ...
        void Panel_M5HDMI::config_resolution(const config_resolution_t& cfg_reso)
        {
            ...
            if (output_width)
            {
                if (output_width > RANGE_MAX)
                {
                    output_width = RANGE_MAX;
                }
                if (logical_width > output_width)
                {
            ...
            if (output_height)
            {
                if (output_height > RANGE_MAX)
                {
                    output_height = RANGE_MAX;
                }
                if (logical_height > output_height)
                {
            ...
            else if (logical_height == 0)
            {
                logical_height = (logical_width * 9) >> 4;
            }
            if (logical_width > RANGE_MAX)
            {
                logical_width = RANGE_MAX;
            }
            if (logical_height > RANGE_MAX)
            {
                logical_height = RANGE_MAX;
            }

            if (refresh_rate > 512.0f)
            {
                refresh_rate = 512.0f;
            }
            else if (refresh_rate < 1.0f)
            {
                ...
                else if (total > 1024000) // over 1280x800
                {
                    refresh_rate = 30.0f;
                }
                else
            ...
            if (output_width == 0 && output_height == 0 && scale_w == 0 && scale_h == 0)
            {
                ...
                if ((scale_w > SCALE_MAX) || (scale_h > SCALE_MAX) || (limit != 1280 * 720) ||
                    (scale_w * logical_width != 1280) || (scale_h * logical_height != 720))
                {
                    scale_w = 1;
                    scale_h = 1;
                    for (int scale = 2; scale <= SCALE_MAX; ++scale)
                    {
                        ...
                        if (scale_width > 1920 || scale_height > 1920 || total > limit)
                        {
                            break;
                        }
                        scale_w = scale;
                ...
                if ((output_width & 1) && (scale_w & 1))
                {
                    output_width += scale_w;
                }
            }
            else
            {
                ...
                if (scale_h > SCALE_MAX)
                {
                    scale_h = SCALE_MAX;
                }
                while (logical_height * scale_h > output_height)
                {
                    --scale_h;
                }
                ...
                while (scale_w > 1 &&
                       (scale_w > SCALE_MAX || w * scale_w != output_width || logical_width * scale_w > output_width))
                {
                    w = output_width / --scale_w;
                }
            }
            if (_pixel_clock > base_clock && scale_w >= 2)
            { // use_half_clock
                scale_w >>= 1;
                output_width >>= 1;
            }
        ...
        color_depth_t Panel_M5HDMI::setColorDepth(color_depth_t depth)
        {
            ...
            if (bits > 16)
            {
                depth = color_depth_t::rgb888_3Byte;
            }
            else if (bits < 16)
            {
                depth = color_depth_t::rgb332_1Byte;
            }
            else
            {
                depth = color_depth_t::rgb565_2Byte;
            }
        ...
        void Panel_M5HDMI::writeFillRectPreclipped(
        ...
        void Panel_M5HDMI::writeFillRectAlphaPreclipped(
            uint_fast16_t x, uint_fast16_t y, uint_fast16_t w, uint_fast16_t h, uint32_t argb8888)
        {
        ...
        void Panel_M5HDMI::_set_window(
            uint_fast16_t xs, uint_fast16_t ys, uint_fast16_t xe, uint_fast16_t ye, uint_fast8_t cmd_write)
        {
            union cmd_t
            {
                uint8_t raw[11];
        ...
        void Panel_M5HDMI::_rotate_pixelcopy(uint_fast16_t& x,
                                             uint_fast16_t& y,
                                             uint_fast16_t& w,
                                             uint_fast16_t& h,
                                             pixelcopy_t* param,
...
#pragma GCC diagnostic pop
        ...
        void
        Panel_M5HDMI::writeImageARGB(uint_fast16_t x, uint_fast16_t y, uint_fast16_t w, uint_fast16_t h, pixelcopy_t* param)
        {
            // ToDo:unimplemented
        }

        void Panel_M5HDMI::readRect(
        ...
        void Panel_M5HDMI::copyRect(uint_fast16_t dst_x,
                                    uint_fast16_t dst_y,
                                    uint_fast16_t w,
                                    uint_fast16_t h,
                                    uint_fast16_t src_x,
                                    uint_fast16_t src_y)
        {
            ...
            if (r)
            {
                if ((1u << r) & 0b10010110)
                {
                    src_y = _height - (src_y + h);
                    dst_y = _height - (dst_y + h);
                }
                if (r & 2)
                {
                    src_x = _width - (src_x + w);
                    dst_x = _width - (dst_x + w);
                }
                if (r & 1)
                {
